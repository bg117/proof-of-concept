...
fatfs::file_allocation_table::impl::impl(const std::string_view path)
    : m_bpb()
{
    ...
    m_total_sectors     = m_bpb.total_sectors_16 == 0
...
void fatfs::file_allocation_table::impl::write_file(
    ...
    const std::vector<std::byte> &data)
{
    ...
    entry.attributes = static_cast<uint8_t>(priv::archive);
    ...
    switch (m_version)
    {
    case file_system_version::fat12: end_of_chain_marker = 0x0FFF;
        break;
    case file_system_version::fat16: end_of_chain_marker = 0xFFFF;
        break;
    case file_system_version::fat32: end_of_chain_marker = 0x0FFFFFFF;
        break;
    }
    ...
    for (std::size_t i = 0; i < path_components.size(); ++i)
    {
        ...
        auto entry = std::find_if(
            ...
            [&](const priv::dir_entry &y)
            {
                ...
                if (i < path_components.size() - 1 || is_directory)
                {
                    return v1 && v2 &&
                           y.attributes &
                           static_cast<int>(priv::directory);
                }
        ...
        if (i != path_components.size() - 1)
        {
            ...
            const std::size_t len =
                contents.size() / sizeof(priv::dir_entry);
            const auto arr =
                reinterpret_cast<const priv::dir_entry *>(ptr);
    ...
    const std::size_t cluster_number) const
{
    ...
    break;
    case file_system_version::fat16: cluster =
                                     reinterpret_cast<const std::uint16_t *>(
                                         m_fat.data())[cluster];
        break;
    case file_system_version::fat32: cluster =
    ...
    const std::string_view path)
{
    ...
    if (trim_string(path.data()) == "\\")
    {
        if (m_version != file_system_version::fat32)
        {
            ...
            raw_dir.resize(m_bpb.root_dir_entries *
                           sizeof(priv::dir_entry));
    ...
    else
    {
        ...
        const std::size_t len =
            contents.size() /
            sizeof(priv::dir_entry); // amount of entries
        const auto arr =
